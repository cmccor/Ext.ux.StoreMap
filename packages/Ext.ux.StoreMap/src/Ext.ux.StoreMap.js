...
Ext.define('Ext.ux.StoreMap', {
    ...
    config: {
        ...
        /**
         * @cfg {Boolean} fitMapsToBounds True if you would like you use Google Maps LatLngBounds to show all the points you have poltted within the bounds of your displayed map.
         * This will also centre your screen on these points.
         */
        fitMapToBounds: true,

        /**
         * @cfg {Boolean} minimumMapZoom True if you wish your map to have a minimum default zoom, as defined by Google Maps' 'mapOptions'.
         * Used in conjunction with fitMapToBounds to prevent zooming maximum zoom on markers.
         * You can adjust the level of this minimum zoom using the minimumZoomLevel config option.
         */
        minimumMapZoom: true,

        /**
         * @cfg {Integer} minimumZoomLevel, use an integer to define the minimum zoom level you would like Google Maps to go to when using fitMapToBounds.
         * minimumMapZoom must be set to true.
         * Defaults to 16.
         */
        minimumZoomLevel: 16,
    ...
    /**
     * Fits the Maps to the bounds of the Lat and Lng that have been passed to it.
     * That Map will centre on the bounds that have been created.
     * @param markers
     */
    fitMapToBounds: function (markers) {

        var latlngbounds = new google.maps.LatLngBounds();

        for (var v = 0; v < markers.getCount(); v++) {

            latlngbounds.extend(new google.maps.LatLng(markers.getAt(v).position.lat(), markers.getAt(v).position.lng()));
            this.getMap().fitBounds(latlngbounds);
        }

        if (this.getMinimumMapZoom()) {
            this.minimumMapZoom();
        }
    },

    /**
     * Map will be provided with a minimum zoom value.
     * This value is defined by the 'minimumZoomLevel' config
     */
    minimumMapZoom: function () {

        var map = this.getMap();
        var me = this;

        google.maps.event.addListenerOnce(map, "idle", function () {
            if (map.getZoom() > me.getMinimumZoomLevel()) {
                map.setZoom(me.getMinimumZoomLevel());
            }
        });
    },
